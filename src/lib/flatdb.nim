...
proc queryDb*(db: FlatDb, matcher: Matcher ): seq[JsonNode] =
  let settings = newQuerySettings()
  queryImpl(db.queryIter, settings, matcher)

proc queryDb*(db: FlatDb, settings: QuerySettings, matcher: Matcher ): seq[JsonNode] =
  queryImpl(db.queryIter, settings, matcher)
...
# ----------------------------- Matcher -----------------------------------------
proc findDb*(key: string, val: string): Matcher {.inline.} =
  return proc (x: JsonNode): bool =
    return x.getOrDefault(key).getStr() == val
proc findDb*(key: string, val: int): Matcher {.inline.} =
  return proc (x: JsonNode): bool  =
    return x.getOrDefault(key).getInt() == val
proc findDb*(key: string, val: float): Matcher {.inline.} =
  return proc (x: JsonNode): bool  =
    return x.getOrDefault(key).getFloat() == val
proc findDb*(key: string, val: bool): Matcher {.inline.} =
  return proc (x: JsonNode): bool  =
...
proc lowerfindDb*(key: string, val: int): Matcher {.inline.} =
  return proc (x: JsonNode): bool = x.getOrDefault(key).getInt <= val
proc lowerfindDb*(key: string, val: float): Matcher {.inline.} =
  return proc (x: JsonNode): bool = x.getOrDefault(key).getFloat <= val
...
proc higher*(key: string, val: float): Matcher {.inline.} =
  return proc (x: JsonNode): bool = x.getOrDefault(key).getFloat > val
proc higherfindDb*(key: string, val: int): Matcher {.inline.} =
  return proc (x: JsonNode): bool = x.getOrDefault(key).getInt >= val
proc higherfindDb*(key: string, val: float): Matcher {.inline.} =
  return proc (x: JsonNode): bool = x.getOrDefault(key).getFloat >= val
...
proc between*(key: string, fromVal:int, toVal: int): Matcher {.inline.} =
  return proc (x: JsonNode): bool =
    ...
    val > fromVal and val < toVal
proc betweenfindDb*(key: string, fromVal:float, toVal: float): Matcher {.inline.} =
  return proc (x: JsonNode): bool =
    let val = x.getOrDefault(key).getFloat
    val >= fromVal and val <= toVal
proc betweenfindDb*(key: string, fromVal:int, toVal: int): Matcher {.inline.} =
  return proc (x: JsonNode): bool =
    let val = x.getOrDefault(key).getInt
    val >= fromVal and val <= toVal
